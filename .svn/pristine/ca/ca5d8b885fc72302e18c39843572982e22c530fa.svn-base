...
public class FunctionController extends BaseController {
	...
	@ResponseBody
	public List<TreeGrid> functionTreeGrid(HttpServletRequest request,TreeGrid treegrid,DataGrid dataGrid) {
		String sqltemp="";
		CriteriaQuery cq = new CriteriaQuery(TSFunction.class);
		String selfId = request.getParameter("selfId");
		if(selfId != null){
		...
		if (treegrid.getId() != null) {
			cq.eq("TSFunction.id", treegrid.getId());
			sqltemp+=" and parentfunctionid='"+treegrid.getId()+"'";
		}
		if (treegrid.getId() == null) {
			cq.isNull("TSFunction");
			sqltemp+=" and (parentfunctionid is NULL)";
		}
		cq.addOrder("functionOrder", SortDirection.asc);
		cq.add();
		List<Map<String, Object>> list=systemService.findForJdbc("select ID,functionlevel,functionname,functionOrder,functionUrl,parentfunctionid from t_s_function where 1=1 "+sqltemp+" order by functionOrder asc", dataGrid.getPage(), dataGrid.getRows());
		List<TSFunction> functionList=new ArrayList<TSFunction>();
		if(list.size()>0){
			for(int i=0;i<list.size();i++){
				Map<String,Object> map=list.get(i);
				TSFunction function=new TSFunction();
				function.setId(String.valueOf(map.get("ID")));
				function.setFunctionLevel(Short.valueOf(map.get("functionlevel").toString()));
				function.setFunctionName(String.valueOf(map.get("functionname")));
				function.setFunctionOrder(String.valueOf(map.get("functionOrder")));
				function.setFunctionUrl(String.valueOf(map.get("functionUrl")));
				functionList.add(function);
			}
		}
		//尝试将这句换换为分页的方式
		//List<TSFunction> functionList1 = systemService.getListByCriteriaQuery(cq, true);
		List<TreeGrid> treeGrids = new ArrayList<TreeGrid>();
		TreeGridModel treeGridModel = new TreeGridModel();
		treeGridModel.setIcon("TSIcon_iconPath");
		treeGridModel.setTextField("functionName");
		...
		treeGridModel.setOrder("functionOrder");
		treeGrids = systemService.treegrid(functionList, treeGridModel);
		//dataGrid.setResults(treeGrids);
		//dataGrid.setRows(functionList.size());
