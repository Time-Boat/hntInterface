...
$(function() {
		$('#user').combotree({
			...
			onClick: function(node){
				$("#userdepartid").val(node.id);
				$("#userdepart").val(node.id);
				$("#user").val(node.name);
			}
		});
	}); 
     ...
     ${user.userName }
		...
		<tr>
			...
			<td align="left" class="value">
  						<input id="user"  type="text" value="${depart.departname}" style="height:24px;width:180px;" >
  							<input id="userdepart"   name="userdepart"  value="${depart.id}" type="hidden">
							<div><input id="userdepartid" name="departid" style="display: none;" value="${depart.TSPDepart.id}"></div>
						</td>
			...
			</select> <span class="Validform_checktip">请选择部门 ${depart.id} }</span></td> --%>
		</tr>
		<tr>
			<td align="right" width="10%" nowrap><label class="Validform_label"> 职务: </label></td>
			<td class="value" width="10%"><input id="userKey" class="inputxt" name="userKey" value="${user.userKey }" > </td>
		</tr>
		<tr>
