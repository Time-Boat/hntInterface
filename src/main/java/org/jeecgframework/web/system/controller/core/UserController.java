...
public class UserController {
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		String password = oConvertUtils.getString(req.getParameter("password"));
		String  departid=oConvertUtils.getString(req.getParameter("userdepart"));
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			depart=(TSDepart)systemService.getEntity(TSDepart.class,departid);
	...
	public void datagridRole(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		TSUser user = ResourceUtil.getSessionUserName();
		if (user.getStatus() != Globals.User_ADMIN && user.getStatus() != -2) {
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			user = systemService.getEntity(TSUser.class, user.getId());
			depart=(TSDepart)systemService.getEntity(TSDepart.class,user.getTSDepart().getId());
			req.setAttribute("user", user);
