...
//			}else{
//				aliasName=alias+"."+origDescriptors[i].getName();
//			}
			aliasName = (alias.equals("") ? "" : alias + ".")+ origDescriptors[i].getName();
			name = origDescriptors[i].getName();
			type = origDescriptors[i].getPropertyType().toString();
			try {
				// judgedIsUselessField(name)这个方法应该是判断当前JavaBean所使用的属性名是否包含此框架中dataGrid中自带的一些属性名
				...
				//这里就很好解释为dataGrid为什么在t标签上写query="",就会出现时间的起始和终止的区间
				...
				if (type.contains("class java.lang")
					...
					if (StringUtils.isNotBlank(beginValue)) {
						...
						} else if (beginValue.length() == 19) {   //yyyy-MM-dd HH:mm:ss
							cq.ge(aliasName, time.parse(beginValue));
						} else if (beginValue.length() == 10) {   //yyyy-MM-dd
