...
@Service("temperaturedataServiceI")
@Transactional
public class W_TemperaturedataServiceImpl extends CommonServiceImpl implements W_TemperaturedataServiceI {
	public JSONObject getDatagrid3(DataGrid dataGrid, String startTime, String endTime, String userlft, String userrgt,
		...
		String sqlCnt = "select count(*) from TemperaturedataView t where"+queryCondition.toString();
		Long iCount = getCountForJdbcParam(sqlCnt, null);
		
		// 取出当前页的数据 
		StringBuffer sql = new StringBuffer();
		sql.append("select * from TemperaturedataView t where"+queryCondition.toString()+"order by tmpshijian asc");
